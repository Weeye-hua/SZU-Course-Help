...
面向深圳大学本科生与研究生选课系统的本地辅助工具，用于登录、浏览课程、维护待处理清单、查看已选课程与周课表，并在学校明确允许的选课时段执行受保护的自动选课任务。本科生支持按校区浏览，研究生使用独立的目录和开放状态判定。

作者：[Weeye](https://github.com/Weeye-hua) · Misakait
</div>

> [!IMPORTANT]
> 本科生预选阶段由学校抽签，本项目会禁止启动自动抢课。研究生按学校开放时间和可选策略判定；两种模式在状态未知、未开放、已结束或无法向学校确认时均禁止启动。请遵守学校规定并自行承担使用责任。

![课程工作台](docs/images/workbench.png)

## 本科生预约启动（v3.7.2 新增）

在“选课清单”的“定时启动”区域，可按学校当前批次公布的开抢时间，或自行输入北京时间预约。支持 30 秒至 7 天后的预约、修改和取消，适合学校已显示“正选”等批次但尚未到实际开放时间的情况。详见下方预约说明和 [使用手册](docs/USER_GUIDE.md)。

预约需要保持本地程序和电脑运行；退出登录、换账号或关闭程序会取消预约，重启不恢复。到点先核验账号、批次、阶段、登录和启用的待抢课程，再进入原有任务流程。预约不绕过学校开放规则，也不保证抢到课程；研究生模式暂不提供此功能。

## 本科生网络入口（v3.7.1 更新）

按照学校提供的本科生选课说明，校内、校外均可直接访问 `http://bkxk.szu.edu.cn`，不需要因为身处校外而选择 WebVPN。建议使用默认“自动”或“主站直连”。访问失败也可能是未开放、维护或网络异常，不能仅凭失败就判断必须使用 VPN。

v3.7.1 接入并补强 [PR #14](https://github.com/Weeye-hua/SZU-Course-Help/pull/14) 的可选 WebVPN 验证码修复：同一轮 token 与图片使用一致入口，并区分网关授权过期与普通验证码加载失败。WebVPN 仍只作为验证码与只读查询备用，不扩展选课提交；学校登录、OCR 自动重登录和自动选课保持主站直连。这不是新增校外登录能力，旧版本也使用相同的学校主站。

## 研究生模式（v3.7.0 新增）

v3.7.0 的源码和各平台 Release 均包含研究生选课入口。运行发布包启动脚本或 `python main.py` 后先选择 `1 本科生` 或 `2 研究生`，再输入学号生成卡密并进入网页。首次登录仍由使用者输入密码并核对验证码；研究生输入四位字符（可点击识别后核对），本科生按顺序点击四个汉字。

- 研究生课程入口：方案内课程、跨专业课程、已选课程、开课查询；支持关键词、院系、满员和冲突筛选，以及课表和未排课列表。
- 开课查询是只读入口。需要选课时，从学校允许选课的方案内或跨专业课程加入本地清单。助手不提供退课操作，也不包含“我的方案”维护。
- 研究生没有本科生的预选/复选/补选阶段划分，也不提供校区切换。是否允许自动选课以学校接口的开放时间、服务器时间、开放标志及选课策略为准；“不可选可退”不能启动。
- 暂停/继续/停止、三种抢课模式、自定义失败阈值（含无限）、自动重登录与严格成功复核共用现有任务引擎。识别验证码默认最多 50 次；连续未知选课响应的默认保护阈值仍为 2000 次。
- 研究生清单保存在运行数据目录的 `graduate/accounts/<账号哈希>/`，不读取或迁移本科生清单。`COURSE_SELECT_DB_PATH` 仅控制本科生清单；研究生自定义位置请设置 `COURSE_SELECT_DATA_DIR`。密码和会话凭据仍仅在进程内存中。
- 课表按节次呈现，不把本科生固定作息时间强行用于研究生。开课查询并不保证该账号具有选课资格，最终限制仍由学校决定。

验证已使用真实账号完成**登录和只读查询**；所有提交、假成功、会话过期等场景均使用模拟响应测试，未进行真实选课或退课。请先核对课程与学校页面一致，再自行决定是否启动任务。
...
启动时先选择本科生或研究生，再输入学号，程序会生成并显示与本机身份绑定的 Card Key；输入 `Y` 后自动启动本地服务并打开对应模式的登录页。首次学校登录仍由用户输入密码并完成验证码。详细步骤见 [Markdown 使用手册](docs/USER_GUIDE.md) 和 [PDF 使用手册](output/pdf/SZU-Course-Help-User-Guide.pdf)。

### v3.7.2 Release 更新

- 整合并补强 [PR #16](https://github.com/Weeye-hua/SZU-Course-Help/pull/16)，实现 [Issue #15](https://github.com/Weeye-hua/SZU-Course-Help/issues/15) 的本科生预约启动需求：学校开抢时间、自定义北京时间、倒计时、修改和取消。
- 学校模式严格匹配当前批次 `beginTime`；提交预约时若时间变化要求重新确认，等待时同步学校调整，到点再次读取。无法确认状态时不会盲目启动。
- 两种预约均绑定账号身份和批次。补齐注销、换号、同账号注销后重登、清单清空、暂停任务存在、手动启动和取消/启动竞争的保护；等待期间沿用现有 OCR 恢复策略。
- 多页面修改及旧程序实例使用版本校验，旧响应不能覆盖新状态。启动核验期间仍可取消，浏览器刷新后恢复预约显示和自定义时间。
- 北京时间使用固定 UTC+8，不额外依赖操作系统时区数据库；学校校时仅作参考，缓存、过慢和过期样本会被丢弃。错过目标超过 2 分钟不会补启动。
- 更新 Markdown/PDF 手册；不改变研究生选课、50 次验证码尝试、2000 次未知响应保护或严格成功复核。继续提供四平台和源码 ZIP，不附带独立 SHA256 文件。

### v3.7.1 Release 更新

- 明确本科生主站校内、校外通用，正常使用保持“自动”或“主站直连”，不把 WebVPN 作为校外访问前提。
- 修复手动选择 WebVPN 后验证码仍固定访问主站的问题，同一轮 token 和图片使用一致入口，网关认证 Cookie 与学校会话 Cookie 分开处理。
- 识别网关认证过期与登录页重定向，不把网络失败、学校未开放等情况一律误报为需要 WebVPN 认证。
- 切换入口时清除旧验证码与点击坐标，请求期间锁定切换和提交，避免旧状态混用。
- 保持学校登录、选课提交和 OCR 自动重登录直连主站；保留 50 次验证码尝试、2000 次未知响应保护、严格成功核验及研究生模式。
- 更新 Markdown/PDF 手册。Release 继续提供 Windows、macOS Apple/Intel 芯片、Linux 与源码 ZIP，不附带独立 SHA256 文件。

### v3.7.0 Release 更新

- 新增研究生独立登录、四位字符验证码及 OCR 自动重登录，启动时可选择本科生或研究生。
- 新增研究生方案内、跨专业、已选与开课查询目录，支持院系筛选及结构化课表；开课查询严格只读。
- 研究生按学校服务器时间、实际开放起止时间、开放标志及可选策略判定，不套用本科生批次名称。
- 复用暂停/继续/停止、爆发/一般/扫描、自定义失败阈值与无限次选项；保留 50 次验证码尝试、2000 次未知响应保护及教学班 ID 严格成功核验。
- 本科生和研究生数据分开，研究生清单与课程缓存按账号隔离；研究生不提供本科生校区切换或 WebVPN 入口。
- 修复扫描目录时会话过期未进入自动重登录的问题。更新跨平台使用手册，本版不包含邮件通知。

### v3.6.4 Release 更新

- 学校返回“添加选课志愿成功”或“已经选过”后不再立即写入本地成功；程序会回查学校已选课程，并且只有完整教学班 ID 精确匹配才确认抢到。
- 已选列表短暂延迟时最多进行 3 次有界回查；列表有效但目标教学班尚未出现时保留课程继续尝试，同名课程或其他教学班不会误判成功。
- 核验时会话过期会触发 OCR 自动重登录；核验接口持续不可用、返回畸形数据或本地状态写入失败时保护性暂停，并在页面显示原因。
- 审查、补强并合入 [PR #13](https://github.com/Weeye-hua/SZU-Course-Help/pull/13)：隐藏冲突/满额后没有可见教学班的课程组会整体隐藏，搜索分页和页面可见数量同步更新，已选教学班始终保留。
- 自动测试增至 292 项，继续拦截未模拟的学校网络请求；选课提交协议、OCR 重登、2000 次未知响应保护和多平台打包规则保持不变。
...
| 本科生/研究生模式 | 启动时选择学校系统，协议、目录和开放状态按模式独立处理 |
| 手动首次登录 | 终端签发学号绑定 Card Key；本科生点击四字验证码，研究生输入四位字符 |
| OCR 自动重登 | 会话过期后自动识别当前模式的验证码，并恢复会话与开放状态 |
| 可见会话恢复 | 工作台显示自动重登录进行中、成功或失败；成功后自动继续原任务 |
| 本科生多校区目录 | 支持粤海、丽湖、深大附属医院、技术大学、香港和深理光明校区；清单逐门保存校区 |
...
| 严格结果确认 | 成功或已选提示必须由学校已选列表中的完整教学班 ID 再确认，拒绝同名或单次文案误判 |
...
- 学校返回成功或“已经选过”后，最多 3 次回查学校已选列表；只有完整教学班 ID 精确匹配才标记 `SUCCESS` 并停止该课程，其余课程继续。
- 容量已满属于正常可重试状态，课程持续保留在活动集，不受“20 次”或未知返回阈值限制。
- 已选提示会先严格核验；时间冲突、超过学分等明确且重试无效的错误才会标记为 `FAILED`。
- 已选列表有效但目标教学班暂未出现时，课程保持活动并继续尝试；列表连续不可用或结构不可信时保护性暂停，绝不按名称、教师或时间模糊确认。
...
在“待处理课程”的“定时启动”区域选择“学校开抢时间”或“自定义时间”。前者从学校 `elective/batch.do` 的当前批次 `beginTime` 读取并显示具体北京时间；若学校未返回可与当前批次对应的时间，此选项不可预约。后者输入北京时间，允许预约 30 秒至 7 天后的时间。两种方式都需要提前勾选阶段确认，并保持本程序运行；预约只保存在本次运行的内存中，退出或注销后失效。

程序优先参考有效学校 HTTP `Date` 响应校时，页面显示当前来源；这不是精密授时，无法取得有效样本时使用本机时间，请开启系统自动校时。到点仍核验学校登录、账号、批次、允许阶段和启用的待抢课程，必要时先按原有预算 OCR 重登录。状态无法确认、批次变化或错过目标超过 2 分钟时不会启动；若提交后学校仍表示未开放，原有任务逻辑会自动暂停，不会绕过限制。

可在同一区域修改或取消预约，核验启动期间也可取消；已真正启动后请使用“停止抢课”。学校模式在创建预约时发现时间变化会要求再次确认，等待期间则同步学校调整后的时间（仍限制在 7 天内）。自定义模式不跟随学校改时间，但同样绑定原批次。网络失败有明确提示，到点无法再次确认时预约失败并保留清单。预约不跨程序重启保存，保持电脑唤醒并留意页面状态。
...
### 为什么学校提示成功，助手却还在核对或继续尝试？

学校的提交响应不一定代表选课已经写入。v3.6.4 会继续读取“我的课程”所用的学校已选列表，并按完整教学班 ID 核对最多 3 次。目标教学班尚未出现时不会误报成功；若核验接口持续不可用，任务会保护性暂停，等待你刷新“我的课程”或恢复后继续。
...
不会。校内外通常都可直接访问主站，WebVPN 不是校外选课的必需条件。WebVPN 认证只为验证码（本地待发布修复）和只读课程、已选信息查询提供备用入口；学校登录和选课提交仍固定发送到主站，不跨后端重试，本版不提供自动退课。即使备用验证码能够获取，也不代表主站登录或选课可用。受控浏览器使用一次性临时配置，关闭后清理，不保存学校密码。
