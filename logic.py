...
def fetch_elective_batch(
    ...
    school_clock.observe(
        headers.get("Date"), received_at, sent_at=sent_at, age_header=headers.get("Age")
    )
    ...
    try:
        ...
        if isinstance(value, (int, float)) or (isinstance(value, str) and value.isdecimal()):
            ...
            if not isinstance(value, str) or not re.match(
                ...
                raise ValueError("a complete date and time is required")
            parsed = datetime.fromisoformat(value.replace("/", "-"))
            if parsed.tzinfo is None:
                parsed = parsed.replace(tzinfo=school_clock.BEIJING)
...
def fetch_undergraduate_start_time(batch_code: str, token: str, combined_cookie: str) -> datetime:
    """Read ``elective/batch.do`` and select the current batch's beginTime."""
    if not str(batch_code).strip() or not token or not combined_cookie:
        raise SchoolStartTimeUnavailableError("请先登录并确认当前选课批次")
    sent_at = time.monotonic()
    response = _school_request(
        "POST",
    ...
    if is_session_expired_response(status_code=response.status_code, code=code, text=response_text):
        raise SchoolBatchSessionExpiredError("学校登录状态已过期")
    response.raise_for_status()
    if isinstance(code, bool) or code not in (None, "1", 1):
        raise SchoolStartTimeUnavailableError("学校未能确认批次时间，请稍后重试")
    ...
    for row in rows:
        ...
        codes = {
            ...
            for value in (
                row.get("code"),
                row.get("batchCode"),
                row.get("electiveBatchCode"),
                nested_code,
            )
            if value is not None and str(value).strip()
        }
        if codes == {str(batch_code).strip()}:
            matching.append(row)
    if len(matching) != 1:
        raise SchoolStartTimeUnavailableError("无法将学校开始时间与当前选课批次对应")
    start = _parse_school_start_time(matching[0].get("beginTime"))
    headers = getattr(response, "headers", {}) or {}
    school_clock.observe(
        headers.get("Date"), received_at, sent_at=sent_at, age_header=headers.get("Age")
    )
